...
<table class="table table-hover table-striped">
    ...
    <tbody>
        ...
        while($dt_histori=mysqli_fetch_array($qry_histori)){
            ...
            $produk_dibeli="<ol>";
            $total=0;
            $qry_produk1=mysqli_query($conn,"select * from  detail_transaksi join produk on produk.id_produk=detail_transaksi.id_produk where id_transaksi = '".$dt_histori['id_transaksi']."'");

            while($dt_produk1=mysqli_fetch_array($qry_produk1)){
                $produk_dibeli.="<li>".$dt_produk1['nama_produk']."</li>";
                $total += $dt_produk1['subtotal'];
            }
